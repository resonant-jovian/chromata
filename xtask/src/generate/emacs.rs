...
fn x11_color(name: &str) -> Option<u32> {
    ...
    X11_COLORS.iter().find(|(n, _)| *n == lower).map(|(_, c)| *c)
}
...
static X11_COLORS: &[(&str, u32)] = &[
    ...
    ("gray10", 0x1a1a1a), ("gray15", 0x262626), ("gray20", 0x333333),
    ("gray25", 0x404040), ("gray30", 0x4d4d4d), ("gray35", 0x595959),
    ("gray40", 0x666666), ("gray50", 0x808080), ("gray60", 0x999999),
    ("gray70", 0xb3b3b3), ("gray75", 0xbfbfbf), ("gray80", 0xcccccc),
    ("gray85", 0xd9d9d9), ("gray90", 0xe5e5e5), ("gray95", 0xf2f2f2),
    ("grey10", 0x1a1a1a), ("grey15", 0x262626), ("grey20", 0x333333),
    ("grey25", 0x404040), ("grey30", 0x4d4d4d), ("grey35", 0x595959),
    ("grey40", 0x666666), ("grey50", 0x808080), ("grey60", 0x999999),
    ("grey70", 0xb3b3b3), ("grey75", 0xbfbfbf), ("grey80", 0xcccccc),
    ("grey85", 0xd9d9d9), ("grey90", 0xe5e5e5), ("grey95", 0xf2f2f2),
];
...
fn map_faces_to_theme(faces: &[FaceDef], name: &str) -> Option<ThemeData> {
    ...
    let bg = face_map.get("default").and_then(|f| f.background.as_ref()).and_then(|s| resolve_color(s))?;
    let fg = face_map.get("default").and_then(|f| f.foreground.as_ref()).and_then(|s| resolve_color(s))?;
    ...
    let get_fg = |face_name: &str| -> Option<u32> {
        face_map.get(face_name).and_then(|f| f.foreground.as_ref()).and_then(|s| resolve_color(s))
    };
    let get_bg = |face_name: &str| -> Option<u32> {
        face_map.get(face_name).and_then(|f| f.background.as_ref()).and_then(|s| resolve_color(s))
    };

    let comment = get_fg("font-lock-comment-face").or_else(|| get_fg("font-lock-comment-delimiter-face"));
